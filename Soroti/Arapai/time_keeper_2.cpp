...
TimerKeeper::TimerKeeper() {
    ...
    ShortDate[0] = '\0';
     ...
     rtc_available = false;
    internal_time_valid = false;
    base_epoch = 0;
    base_micros = 0;
    last_ntp_sync_us = 0;
    memset(&internal_tm, 0, sizeof(internal_tm));
}
...
bool TimerKeeper::initialize_RTC(const uint8_t i2c_dio , const uint8_t i2c_clock) {
     
      Wire.setPins(i2c_dio, i2c_clock); // Set pins BEFORE calling Wire.begin()
...
void TimerKeeper::update() {
    if (rtc_available) {
        query_rtc();   // existing RTC read
    } else if (internal_time_valid) {
        updateInternalTime();
    } else {
        // No time source – leave defaults
    }
}

void TimerKeeper::periodicNtpSync(WiFi_Manager& wifi, long gmtOffset, int daylightOffset) {
    if (rtc_available) return; // RTC keeps time, no need for periodic NTP
    if (!internal_time_valid) return; // haven't set internal time yet

    uint64_t now_us = esp_timer_get_time();
    // Sync every hour (3600 seconds)
    if ((now_us - last_ntp_sync_us) > 3600000000ULL) {
        if (wifi.is_connected()) {
            if (wifi.sync_ntp(gmtOffset, daylightOffset, 5)) {
                // Successful sync – update internal time with new NTP time
                setInternalTime(wifi.ntp_timeinfo);
                last_ntp_sync_us = now_us;
                Serial.println("[TIME] Periodic NTP sync successful.");
            } else {
                Serial.println("[TIME] Periodic NTP sync failed.");
            }
        } else {
            // WiFi not connected; we'll try again later
            last_ntp_sync_us = now_us; // avoid hammering if WiFi is down
        }
    }
}
        ...
        if (now - last_reinit_check >= 3600000) {
            ...
            if (test_time.year() < 2020 || test_time.year() > 2100) {
                ...
                if (consecutive_failures >= 3) {
                    ...
                    clock_is_working = false;
                   // initialize_RTC(); // RECURSIVELY CALL RE-INIT
...
void TimerKeeper::setInternalTime(const struct tm& timeinfo) {
    base_epoch = mktime((struct tm*)&timeinfo); // convert tm to epoch
    base_micros = esp_timer_get_time();
    internal_time_valid = true;
    rtc_available = false;
    // Copy timeinfo to internal_tm for immediate use
    internal_tm = timeinfo;
    // Also set our member variables hr, mint, sec, day_, mth, mwaka
    hr = timeinfo.tm_hour;
    mint = timeinfo.tm_min;
    sec = timeinfo.tm_sec;
    day_ = timeinfo.tm_wday;
    mth = timeinfo.tm_mon + 1;
    mwaka = timeinfo.tm_year + 1900;
    // Format strings
    formatTime12Hour(hr, mint, sec);
    formatLongDate(day_, timeinfo.tm_mday, mth, mwaka);
    formatShortDate(timeinfo.tm_mday, mth, mwaka);
}

void TimerKeeper::updateInternalTime() {
    if (!internal_time_valid) return;
    uint64_t now_micros = esp_timer_get_time();
    int64_t delta_us = now_micros - base_micros;
    time_t now_epoch = base_epoch + (delta_us / 1000000ULL);
    struct tm tm_now;                     // ordinary struct, not a pointer
    localtime_r(&now_epoch, &tm_now);     // &tm_now is tm*, correct

    // Now copy values from tm_now into your member variables
    hr = tm_now.tm_hour;
    mint = tm_now.tm_min;
    sec = tm_now.tm_sec;
    day_ = tm_now.tm_wday;
    mth = tm_now.tm_mon + 1;
    mwaka = tm_now.tm_year + 1900;
    formatTime12Hour(hr, mint, sec);
    formatLongDate(day_, tm_now.tm_mday, mth, mwaka);
    formatShortDate(tm_now.tm_mday, mth, mwaka);
}
