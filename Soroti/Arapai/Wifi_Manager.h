...
private:
   ...
   unsigned long backoff_delay_ms;
    ...
    static constexpr uint64_t RECONNECT_INTERVAL_US = 45ULL * 1000000ULL; // 45 seconds

    // Internal helpers
    void log_message(const char* msg) const;
    void set_led(bool connected) const;
    bool try_connect(const char* ssid, const char* pass, int timeout_ms = 60000);
};
