...
UploadStatus upload::upload_to_web_of_iot(const char* JSON_Bundle) {
    ...
    //client.setInsecure();
    //client.setFollowRedirects(HTTPC_FORCE_FOLLOW_REDIRECTS);  // ← ADD THIS
...
// upload.cpp — stream directly from SD File
UploadStatus upload::upload_file_from_sd(File& file, size_t file_size) {

    Serial.println("=== [Uploader] Begin Streamed SD Upload ===");

    if (file_size == 0) {
        update_report("Upload failed: empty file");
        return UPLOAD_TOO_LARGE; // reuse or add UPLOAD_EMPTY
    }
    ...
    https.setTimeout(30000); // larger file needs more time
    https.addHeader("Content-Type", "text/plain");
    https.addHeader("Content-Length", String(file_size));

    Serial.printf("[Uploader] Streaming %u bytes from SD...\n", (unsigned)file_size);

    // ← This is the magic: reads SD in small chunks internally, ~2KB RAM used
    int httpResponse = https.sendRequest("POST", &file, file_size);
    ...
    if (httpResponse == HTTP_CODE_OK) {
        status = UPLOAD_HTTP_OK;
        update_error("OK");
        Serial.println("[Uploader] ✅ Streamed upload successful!");
    } else if (httpResponse >= 400 && httpResponse < 500) {
        status = UPLOAD_HTTP_CLIENT_ERROR;
        update_error("4xx client error");
    } else if (httpResponse >= 500) {
    ...
    snprintf(upload_report, sizeof(upload_report),
             "Streamed POST: resp=%d, size=%u bytes", httpResponse, (unsigned)file_size);
